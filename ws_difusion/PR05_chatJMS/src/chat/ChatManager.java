...
public class ChatManager implements javax.jms.MessageListener{
	...
	public void open(String dname, String user,String conv, boolean dur, String durname) {
        try {
            ...
            sus_session = connection.createTopicSession(false, Session.AUTO_ACKNOWLEDGE);
            pub_session = connection.createTopicSession(false, Session.AUTO_ACKNOWLEDGE);
            
            publisher = pub_session.createPublisher(dest);
            
            if (durable) subscriber = sus_session.createDurableSubscriber(dest, durablename);
            if (!durable) subscriber = sus_session.createSubscriber(dest);
	...
	public void send(String text, long timespan){
		try {
              //EJERCICIO: 
			TextMessage message = pub_session.createTextMessage();
